  ...
  const section = (title: string, items: typeof exchanges) => (
    <section className="space-y-3">
      ...
      {items.length > 0 && <div className="grid gap-4 lg:grid-cols-2">{items.map(exchange => {
        ...
        return <Link key={exchange.id} href={`/exchanges/${exchange.id}${exchange.status === "completed" ? "/complete" : ""}`} className="group min-w-0 overflow-hidden rounded-[var(--radius-card)] border border-border bg-surface p-5 shadow-subtle transition hover:border-primary/40 hover:shadow-md">
