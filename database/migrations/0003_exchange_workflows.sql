...
begin
  ...
  update exchanges set status='completed',completed_at=p_now,updated_at=p_now,payload=payload || jsonb_build_object('status','completed','escrowStatus','released','providerEscrowStatus',case when is_mutual then 'released' else payload->>'providerEscrowStatus' end,'requesterEscrowStatus',case when is_mutual then 'released' else payload->>'requesterEscrowStatus' end,'providerAcceptedAt',case when v_is_provider then to_jsonb(p_now) else payload->'providerAcceptedAt' end,'requesterAcceptedAt',case when v_is_requester then to_jsonb(p_now) else payload->'requesterAcceptedAt' end,'completedAt',p_now,'updatedAt',p_now) where id=p_exchange_id;
