        ...
        {skillsOffered.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {skillsOffered.map((skill, idx) => (
              <div key={idx} className="bg-background border border-border px-3 py-2.5 rounded-[var(--radius-card)] flex flex-col justify-between shadow-subtle group hover:border-primary/50 transition-colors min-w-0 h-full">
                <div className="flex items-center gap-2 min-h-[40px]">
                  ...
                  <span className="font-bold text-sm text-heading break-words flex-1 leading-tight">{skill.name}</span>
                </div>
        ...
        {skillsLookingFor.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {skillsLookingFor.map((skill, idx) => (
              <span key={idx} className="bg-surface-secondary border border-border text-body text-sm font-medium px-3 py-1.5 rounded-full flex items-center gap-1.5">
