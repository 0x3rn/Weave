...
import { updateMarketplaceRequest } from "@/app/actions/marketplace";
...
export default function EditRequestClient({ request }: { request: MarketplaceRequest }) {
  ...
  const [formData, setFormData] = useState({
    ...
    skillsRequired: request.skillsRequired.join(", "),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateMarketplaceRequest(request.id, requestData);
